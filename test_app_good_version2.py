...
class TestCaseGenerator(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.create_custom_test_section(main_layout)
        self.create_test_generation_section(main_layout)
        self.create_action_section(main_layout)
        

    def create_custom_test_section(self, main_layout):
        """Section for selecting and running custom test files"""
        ...
        # Custom files list
        custom_files_label = QLabel("Custom Test Files:")
        self.custom_files_list = QListWidget()
        
        # Add files button
        add_tests_btn = StyledButton("Add Test Files", "#4CAF50")
        add_tests_btn.clicked.connect(self.select_custom_tests)
        
        # Run Tests button
        run_tests_btn = StyledButton("Run Selected Tests", "#2196F3")
        run_tests_btn.clicked.connect(self.run_custom_tests)

        # Add components to layout
        custom_layout.addWidget(custom_files_label)
        custom_layout.addWidget(self.custom_files_list)
        custom_layout.addWidget(add_tests_btn)
        custom_layout.addWidget(run_tests_btn)

        # Add frame to main layout (adjust grid position as needed)
        main_layout.addWidget(custom_frame, 2, 0, 1, 2)

    def select_custom_tests(self):
        """Open file dialog to select custom test files"""
        ...
        if file_paths:
            ...
            self.custom_files_list.clear()
            for path in file_paths:
                self.custom_files_list.addItem(path)

    def run_custom_tests(self):
        """Run selected test files through analysis"""
        if not self.main_class_path:
            self.show_error_message("Error", "Please select a main class first!")
            return

        if self.custom_files_list.count() == 0:
            self.show_error_message("Error", "Please select test files first!")
            return
        ...
        # Initialize appropriate analyzer based on first file's extension
        first_file = self.custom_files_list.item(0).text()
        if first_file.lower().endswith('.html'):
        ...
        for i in range(self.custom_files_list.count()):
            test_file = self.custom_files_list.item(i).text()
            try:
                # Dynamic analysis
                with open(test_file, 'r', encoding='utf-8') as f:
                    for line_num, line in enumerate(f, 1):
                        if file_type == "HTML":
                            analyzer.check_html_structure(line, line_num)
                        else:
                            analyzer.check_pdf_structure(line, line_num)
                ...
                self.show_error_message("Warning", f"Error processing file {test_file}: {str(e)}")
    ...
    def create_action_section(self, main_layout):
        ...
        buttons = [
            ...
            ("Download Report", "#4CAF50", self.download_report) 
        ]
    ...
    def download_report(self):
        """Bundle coverage reports, analysis files, and generated test cases into a PDF with links"""
        if not self.main_class_path:
            self.show_error_message("Error", "Please select a main class first!")
            return
        ...
        try:
            ...
            output_pdf = os.path.join(working_dir, f"complete_report_{timestamp}.pdf")
            
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", "B", 16)
            pdf.cell(0, 10, "Complete Analysis Report", ln=True, align='C')
            
            # Add coverage report links
            coverage_dir = os.path.join(working_dir, "coverage-report", "default")
            if os.path.exists(coverage_dir):
                pdf.add_page()
                pdf.set_font("Arial", "B", 14)
                pdf.cell(0, 10, "Coverage Reports (Click to Open)", ln=True)
                pdf.set_font("Arial", size=12)
                
                # Add links to HTML files with blue color
                pdf.set_text_color(0, 0, 255)
                for file in os.listdir(coverage_dir):
                    if file.endswith('.html') and not file.endswith('.js.html'):
                        file_path = os.path.join(coverage_dir, file)
                        pdf.cell(0, 10, file, ln=True, link=file_path)
                pdf.set_text_color(0, 0, 0)
            
            # Add generated test case links based on file type
            file_type = self.file_type_combo.currentText()
            if file_type == "HTML":
                generated_dir = os.path.join(working_dir, "generated_html_preprocessed")
                file_ext = ".html"
            else:  # PDF
                generated_dir = os.path.join(working_dir, "generated_pdf")
                file_ext = ".txt"
                
            if os.path.exists(generated_dir):
                pdf.add_page()
                pdf.set_font("Arial", "B", 14)
                pdf.cell(0, 10, f"Generated {file_type} Test Cases (Click to Open)", ln=True)
                pdf.set_font("Arial", size=12)
                
                # Add links to generated files with blue color
                pdf.set_text_color(0, 0, 255)
            ...
            for file in os.listdir(working_dir):
                if file.startswith(('analysis_result_', 'html_dynamic_', 'pdf_dynamic_')):
                    pdf.add_page()
                    pdf.set_font("Arial", "B", 14)
                    pdf.cell(0, 10, f"Report: {file}", ln=True)
                    pdf.set_font("Arial", size=12)
            ...
            self.show_error_message("Success", f"Report downloaded to:\n{output_pdf}")
            
        except Exception as e:
            self.show_error_message("Error", f"Failed to create report: {str(e)}")
